...
describe("writeGeneratedTokens (the core of build:theme)", () => {
  ...
  it("writes only the primitives file (semantics owned in Figma)", () => {
    expect(existsSync(join(dir, "primitives-color.mode-1.tokens.json"))).toBe(true);
    expect(existsSync(join(dir, "color.light.tokens.json"))).toBe(false);
    expect(existsSync(join(dir, "color.dark.tokens.json"))).toBe(false);
  });
