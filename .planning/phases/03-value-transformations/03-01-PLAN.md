...
**Concrete before/after for the root build (Build 1, line ~82):**
...
AFTER (with sharedPlatformConfig applied):
```typescript
const sdRoot = new StyleDictionary({
  source: rootSources,
  log: { warnings: 'disabled' },
...
The ONLY change per build: replace `transformGroup: 'css/extended',` with `...sharedPlatformConfig,`. Apply this same replacement to all 7 builds:
- Build 1: `sdRoot` (line ~82) — `:root` selector
- Build 2: `sdLight` (line ~111) — `[data-color-mode='light']` selector
- Build 3: `sdDark` (line ~145) — `[data-color-mode='dark']` selector
- Builds 4-7: `sdRadius` inside the `for` loop (line ~175) — `[data-radius-mode='*']` selectors

Each build keeps its own unique `source`, `buildPath`, `files` (with destination, filter, selector). Only `transformGroup: 'css/extended'` is replaced by the spread.
...
9. **XFRM-01 (transform order):** In `scripts/buildTokens.ts`, verify `dimension/unitless` appears BEFORE `dimension/css` in the `sharedPlatformConfig.transforms` array. Run: `grep -n 'dimension/' scripts/buildTokens.ts` — `dimension/unitless` line number must be lower than `dimension/css` line number. This ordering is critical because `dimension/css` would otherwise add px units before `dimension/unitless` can strip them.
</verification>
